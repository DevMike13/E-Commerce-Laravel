        ...
        <div class="w-1/2 h-full flex items-center justify-center flex-col gap-2">
            <div class="flex flex-col bg-white border border-t-4 border-t-green-600 shadow-sm rounded-xl dark:bg-slate-900 dark:border-gray-700 dark:border-t-blue-500 dark:shadow-slate-700/[.7]">
                <div class="p-4 md:p-5 text-end">
                    <h1 class="text-3xl font-semibold mb-9 text-center">{{ __('trans.login') }}</h1>
                    <form wire:submit.prevent="login">
                        ...
                        <a href="/forgot" class="text-blue-600 underline text-end">{{ __('trans.forgot') }}</a>
                        <x-button rounded positive label="{{__('trans.login')}}" class="w-full mt-8 text-xl" type="submit" />
                    </form>
                    <p class="text-center mt-5 text-sm">{{ __('trans.dont-have-account') }} <a href="/register" class="text-blue-600 underline">{{ __('trans.signup-here')}}</a></p>
                </div>
            </div>
        </div>
    </div>
